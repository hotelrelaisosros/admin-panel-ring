...
const BirthStone = () => {
  const apiUrl = '/api/v1/products/birthstones';
  const [items, setItems] = useState([]);
  const [editingItem, setEditingItem] = useState(null);
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [form] = Form.useForm();
  const key = "BirthStones"
  ...
  const onFinish = (values) => {
    if (editingItem) {
      apiService.updateItem(apiUrl, editingItem.id, values, setItems,"BirthStone");
    } else {
      apiService.addItem(apiUrl, values, setItems,"BirthStone");
    }
    handleCancel();
  };
  ...
  const gemStoneFields = [
    { name: 'name', label: 'Birthstone Name', type: 'input', rules: [{ required: true, message: 'Please input the birthstone name!' }] },
    ...
    <div>
      <Card>
        <Space style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <Title level={3} style={{ marginBottom: 0 }}>BirthStone</Title>
          <Button type="primary" onClick={showModal}>Add Birthstone</Button>
        </Space>
