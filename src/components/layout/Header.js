...
import { Badge, Button, Col, Modal, Popover, Row, Tooltip } from "antd";
import { useEffect, useState } from "react";
import { useDispatch } from "react-redux";
import { useHistory } from "react-router-dom";
import "../../firebaseConfig";
import { NotificationRealTimeMiddleware } from "../../store/notificationRealTime/notificationRealTimeMiddleware";
...
export default function Header() {
  ...
  // //Modal logout state
  // const [previewOpenLogout, setPreviewOpenLogout] = useState(false);
  const { confirm } = Modal;
  //Modal logout state
  ...
  //   }
  // };
  // useEffect(() => window.scrollTo(0, 0), checktoken());
  ...
  useEffect(() => {
    onMessageListener();
    dispatch(
      NotificationRealTimeMiddleware.GetNotificationRealTime(token)
    ).then((res) => {
      console.log("res=>>>>", res?.notifications_count);
      setDataNotify(res);
    });
  }, []);
  ...
  const onMessageListener = () => {
    ...
    onMessage(messaging, (payload) => {
      ...
      console.log("on message token =>", token);
    });
    // });
  };
  const notification_data = dataNotify?.Notifications;
  console.log("dataNotify=SSS>", dataNotify);
  const text = (
  ...
  const content = (
    <>
      {/* {console.log(notification_data, "notichecking")} */}
      {notification_data?.map((msg_notify) => {
        return (
          <ul>
            <li className="list-style-none">
              <h6>
                <b>{msg_notify?.navigation}</b>
              </h6>
              <p className="">{msg_notify?.notification}</p>
            </li>
          </ul>
        );
      })}
    ...
    <>
      ...
      <Row gutter={[24, 0]}>
        ...
        <Col span={24} md={18} className="header-control">
          ...
          <span className="btn-sign-in">
            <Tooltip title="Profile">
              <Button
                className="border-0 bg-transparent text-white"
                onClick={() =>
                  history.push({ pathname: "/userProfile", state: {} })
                }
              >
                <PersonOutline />
              </Button>
            ...
            >
              <Tooltip title="Notifications">
                <Button className="border-0 bg-transparent text-white">
                  <Badge
                    count={dataNotify?.notifications_count}
                    overflowCount={10}
                  >
                    <NotificationImportantOutlined />
                  </Badge>
